...
def cloudwatch_put_metric(size_category, duration_ms, detection_count):
    """
...
def process_image(bucket, key):
    ...
    cloudwatch_put_metric(size_category, duration_ms, len(detections))

    logger.info(f"Processed s3://{bucket}/{key} in {duration_ms:.2f} ms, size category: {size_category}, detections: {len(detections)}")
